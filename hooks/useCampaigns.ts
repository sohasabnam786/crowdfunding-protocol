...
export function useCreateCampaign() {
  ...
  return useMutation({
    mutationFn: async (params: CreateCampaignParams) => {
      ...
      toast.loading(`Creating campaign: ${params.title}`, { id: txId });
  ...
  return useMutation({
    mutationFn: async (amountXlm: number) => {
      ...
      const txId = addTransaction({
        ...
        campaignId,
        amount: amountXlm,
        description: `Donating ${amountXlm} XLM to Campaign #${campaignId}`,
      });

      toast.loading(`Donating ${amountXlm} XLM...`, { id: txId });
      ...
      try {
        ...
        toast.success(`Donated ${amountXlm} XLM!`, {
          id: txId,
          description: `Tx: ${result.txHash.slice(0, 16)}...`,
        });
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const txId = addTransaction({
        ...
        status: "pending",
        campaignId,
        description: `Withdrawing funds from Campaign #${campaignId}`,
      });

      toast.loading("Withdrawing funds...", { id: txId });
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const txId = addTransaction({
        ...
        status: "pending",
        campaignId,
        description: `Requesting refund from Campaign #${campaignId}`,
      });
