...
/// ────────────────────────────────────────────────────────────────────────────
/// CrowdFund Reward Token (CRWD)
///
/// A simple fungible token contract used to reward donors on the CrowdFund
/// Protocol.  Only the designated `admin` (set during initialization — the
/// CrowdFund contract itself) may mint new tokens.  Token holders can
...
impl RewardTokenContract {
    ...
    /// Initialize the token with an admin (typically the CrowdFund contract).
    ///
    ...
    pub fn initialize(
        ...
        name: String,
        symbol: String,
    ) -> Result<(), RewardTokenError> {
        if env.storage().instance().has(&DataKey::Admin) {
            return Err(RewardTokenError::AlreadyInitialized);
        }
    ...
    ///
    /// Only the admin (CrowdFund contract) may call this.
    pub fn mint(
        env: Env,
        to: Address,
        amount: i128,
        ...
        let admin: Address = env
            ...
            .instance()
            .get(&DataKey::Admin)
            .ok_or(RewardTokenError::NotInitialized)?;
        admin.require_auth();

        if amount <= 0 {
            return Err(RewardTokenError::InvalidAmount);
        }
    ...
    pub fn transfer(
        ...
        if from_balance < amount {
            return Err(RewardTokenError::InsufficientBalance);
        }
    ...
    /// Returns the CRWD balance of `owner`.
    pub fn balance_of(env: Env, owner: Address) -> i128 {
        Self::balance_internal(&env, &owner)
    }

    /// Returns the token name (e.g. "CrowdFund Reward").
    pub fn name(env: Env) -> String {
        env.storage()
            .instance()
            .get(&DataKey::Name)
            .unwrap_or_else(|| String::from_str(&env, "CrowdFund Reward"))
    }

    /// Returns the token symbol (e.g. "CRWD").
    pub fn symbol(env: Env) -> String {
        env.storage()
            .instance()
            .get(&DataKey::Symbol)
            .unwrap_or_else(|| String::from_str(&env, "CRWD"))
    }

    /// Returns the admin address.
    pub fn admin(env: Env) -> Result<Address, RewardTokenError> {
        env.storage()
            .instance()
            .get(&DataKey::Admin)
            .ok_or(RewardTokenError::NotInitialized)
...
mod test {
    ...
    fn setup() -> (Env, RewardTokenContractClient<'static>, Address) {
        ...
        client.initialize(
            &admin,
            &String::from_str(&env, "CrowdFund Reward"),
    ...
    fn test_initialize_and_metadata() {
        ...
        assert_eq!(client.name(), String::from_str(&env, "CrowdFund Reward"));
    ...
    fn test_mint_increases_balance() {
        ...
        client.mint(&recipient, &500_000_000_i128); // 50 CRWD (in stroops)
