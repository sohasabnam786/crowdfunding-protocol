...
import { Heart, Loader2, AlertCircle, Zap } from "lucide-react";
  ...
  return (
    <div className="glass-card p-6 space-y-5">
      <h3 className="font-bold text-base">
        {isActive ? "Back This Campaign" : isExpired ? "Request Refund" : "Campaign Details"}
      </h3>
      ...
      {isActive && (
        <>
          {/* Quick amounts */}
          <div>
            <p className="text-xs text-muted-foreground mb-3">Quick select amount (XLM)</p>
          ...
          {amount > 0 && (
            <div className="px-4 py-3 rounded-lg bg-primary/5 border border-primary/10">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">You donate</span>
            ...
            >
              {isDonating ? (
                ...
                <>
                  ...
                  <span>Donate {amount > 0 ? formatXlm(amount) : ""}</span>
                </>
              )}
            </button>
          ) : (
        ...
        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
          ...
          <div>
            <p className="text-sm font-medium text-emerald-400">
              {campaign.status === "Withdrawn" ? "Funds Withdrawn" : "Goal Reached!"}
            </p>
            <p className="text-xs text-emerald-400/70 mt-0.5">
              {campaign.status === "Withdrawn"
                ? "The creator has successfully withdrawn all funds."
                : "This campaign has met its funding goal. Creator can now withdraw."}
            </p>
          </div>
        </div>
      )}
